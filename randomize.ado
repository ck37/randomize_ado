...
* Example syntax: randomize
 - optional settings:
	...
	minruns() -> minimum number of randomizations to run per stratum, default to 10.
	maxruns() -> maximum number of randomizations to run per stratum, default to 10.
	...
	replace -> specify if the assignment variable will be replaced upon generation.
...
- Convert local variables to tempvars so that we don't pollute the global namespace.
...
- Allow percentage breakdown between assignment groups (e.g. 70%/20%/10%) but then don't do automatic re-randomization.
- Support cluster randomization within the algorithm eventually.

====================================*/



syntax [if] [in] [, GRoups(integer 2) MINruns(integer 10) MAXruns(integer 10) BALance(varlist) BLock(varlist) COEFFthreshold(real 0.05) JOintp(real 0.5) GENerate(name) MODel(string) seed(real 37) REPlace]
